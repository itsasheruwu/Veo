...
final class DesktopCodexStore: ObservableObject {
    ...
    init(
        ...
        resolvedClient.serverRequestHandler = { [weak self] request, route in
            self?.captureServerRequest(request, route: route) ?? false
        }
    ...
    private func loadVeoThreadsFromStore(archived: Bool? = nil) async {
        do {
            let active = try await threadStore.listThreads(archived: false)
            let archivedRows = try await threadStore.listThreads(archived: true)
    ...
    private func loadThreads(
        ...
        do {
            ...
            let parsed = rows.compactMap { DesktopThread.parse($0, origin: .codex) }
            ...
            if let threadObject {
                if let existing = findThread(uiThreadID), existing.origin == .veo {
                    let updated = DesktopThread.makeVeo(
                        ...
                        title: threadObject.string("name") ?? existing.title,
    ...
    private func refreshSelectedVeoThreadMetadata() {
        ...
        if shouldReplaceTitle, let titleSeed, !titleSeed.isEmpty {
            nextTitle = String(titleSeed.prefix(68))
        } else {
